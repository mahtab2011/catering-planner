...
function handleRemoveStaffFromEvent(staffId: string) {
  ...
  for (const item of mergedItems) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (price > 0) {
        totals[i] += price * (Number(item.qty) || 0);
      }
    }
  }
...
const suggestedTotalRevenue =
  ...
  async function persistTotalsAndLists() {
    ...
    const newTotalCost = currentCostTotal + totalLabourCost;
  ...
  updatedAt: serverTimestamp(),
      ...
      setSupplierNames(
        Array.isArray(normalized?.supplierNames) &&
          ...
          ? [...normalized.supplierNames, "Supplier A", "Supplier B", "Supplier C", "Supplier D", "Supplier E"].slice(0, 5)
...
<div className="rounded-2xl border border-sky-200 bg-white p-4 shadow-sm">
  <div className="flex items-center justify-between gap-3">
    <div>
      <h3 className="text-lg font-semibold text-neutral-900">Event Staff Labour</h3>
      <p className="text-sm text-neutral-600">
        Assign staff to this event and calculate labour cost automatically.
      </p>
    </div>
  </div>
    ...
    <div className="flex flex-wrap gap-2">
      {availableStaff.map((staff) => (
        <button
          key={staff.id}
          type="button"
          onClick={() => handleAddStaffToEvent(staff)}
          className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-medium text-sky-800 hover:bg-sky-100"
        >
          + {staff.name} ({String(staff.role || "").replace("_", " ")})
        </button>
      ))}
    </div>
  </div>
  ...
  <div className="mt-4 space-y-3">
    {staffAssignments.length === 0 ? (
      ...
      staffAssignments.map((row) => (
        <div
          key={row.staffId}
          className="grid gap-3 rounded-xl border border-neutral-200 bg-neutral-50 p-4 md:grid-cols-6"
        >
          ...
          <div>
            ...
            <input
              ...
              step="0.5"
              value={row.hours}
              onChange={(e) => handleUpdateStaffHours(row.staffId, e.target.value)}
              className="w-full rounded-xl border border-neutral-300 px-3 py-2 text-sm outline-none focus:border-sky-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={`£${Number(row.totalCost || 0).toFixed(2)}`}
              readOnly
              className="w-full rounded-xl border border-neutral-300 bg-neutral-100 px-3 py-2 text-sm outline-none"
            />
          </div>
          ...
          <div className="flex items-end">
            <button
              type="button"
              onClick={() => handleRemoveStaffFromEvent(row.staffId)}
  ...
  <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
    <div className="text-sm font-medium text-neutral-700">Total Staff Labour Cost</div>
    <div className="mt-1 text-2xl font-bold text-emerald-700">
      £{totalStaffLabourCost.toFixed(2)}
    </div>
  </div>
</div>
      setSaveState("saved");
      setEventData((prev) =>
        prev ? { ...prev, status: "closed", lockedAt: new Date() } : prev
      );
...
if (raw?.staffAssignments) {
  ...
  try {
    ...
    bestPurchasePlan.forEach((row: any) => {
    ...
    const text = Object.entries(grouped)
      .map(([supplier, rows]) => {
        const lines = rows.map(
          (row: any, idx: number) =>
            `${idx + 1}. ${row.ingredientName} - Qty: ${row.qty} ${row.unit} @ £${Number(
              row.unitPrice || 0
            ).toFixed(2)} = £${Number(row.lineTotal || 0).toFixed(2)}`
        );
...
Optimized Total Cost: £${Number(optimizedTotalCost || 0).toFixed(2)}
Total Event Savings: £${Number(totalEventSavings || 0).toFixed(2)}
Best Supplier: ${bestSupplier?.name || "-"}
Best Supplier Saving: £${Number(bestSupplier?.saving || 0).toFixed(2)}
`;

    saveAllSuppliersOrderLog({
      message: pdfSummary.trim(),
      sentVia: "pdf",
    });
    ...
    const safeRows = bestPurchasePlan.map((row: any, idx: number) => {
      const qty = Number(row?.qty || 0);
    ...
    safeRows.forEach((row) => {
      ...
      supplierTotalsMap[supplier] =
        Number(supplierTotalsMap[supplier] || 0) + Number(row.lineTotal || 0);
    });
  ...
  try {
    ...
    const message = Object.entries(grouped)
      .map(([supplier, rows]) => {
        const lines = rows.map(
          (row: any, idx: number) =>
            `${idx + 1}. ${row.ingredientName} - ${row.qty} ${row.unit} @ £${Number(
              row.unitPrice || 0
            ).toFixed(2)} = £${Number(row.lineTotal || 0).toFixed(2)}`
        );
...
function buildSupplierMessages(): Record<string, string> {
  try {
    if (!Array.isArray(bestPurchasePlan) || bestPurchasePlan.length === 0) {
      return {};
    }
    ...
    bestPurchasePlan.forEach((row: any) => {
      const supplier = String(row?.supplierName || "Unknown Supplier");
    ...
    Object.entries(grouped).forEach(([supplier, rows]) => {
      ...
      const lines = rows.map((row: any, idx: number) => {
        const ingredientName = String(row?.ingredientName || `Item ${idx + 1}`);
        const qty = Number(row?.qty || 0);
        const unit = String(row?.unit || "");
        
        // ✅ important fallback (prevents undefined issues)
...
${bestPurchasePlan
  .map(
    (row: any, i: number) => `
${i + 1}. ${row.ingredientName || "-"}
Qty: ${Number(row.qty || 0)} ${row.unit || ""}
...
async function handleWhatsAppSupplier(supplier: string) {
  try {
    ...
    const supplierRows = (bestPurchasePlan || []).filter(
      (row: any) => String(row?.supplierName || "") === supplier
    );

    const supplierTotal = supplierRows.reduce((sum: number, row: any) => {
      const qty = Number(row?.qty || 0);
      const unitPrice = Number(row?.unitPrice ?? row?.price ?? 0);
      const lineTotal = Number(row?.lineTotal ?? qty * unitPrice);
...
async function handleExportSupplierPDF(supplier: string) {
  try {
    ...
    // your existing PDF content generation here...
...
async function logActivity({
  ...
  try {
    ...
    console.log("logActivity success, doc id:", docRef.id);
  } catch (err) {
    console.error("Logging failed", err);
    alert("Logging failed - check console");
  }
}
async function saveSupplierOrderLog({
  ...
  total: number;
  message: string;
  sentVia: "whatsapp_single" | "whatsapp_all_one" | "whatsapp_all_separate" | "copy" | "pdf";
}) {
  try {
    await addDoc(collection(db, "supplier_orders"), {
      ...
      rows: rows.map((row: any) => ({
        ...
        lineTotal: Number(
          row?.lineTotal ??
            Number(row?.qty || 0) * Number(row?.unitPrice ?? row?.price ?? 0)
        ),
      })),
      total: Number(total || 0),
      message: message || "",
    ...
    alert(`Saved supplier order log for ${supplier}`);
  } catch (err: any) {
...
async function saveAllSuppliersOrderLog({
  ...
  try {
    const rows = Array.isArray(bestPurchasePlan) ? bestPurchasePlan : [];
...
/* =======================
  ...
  const cheapestGrandTotal: number = cheapestSupplier
    ? Number(
        supplierTotals[Number(cheapestSupplier) as keyof typeof supplierTotals] ??
          0
      )
    : 0;
...
const totalLabourCost = useMemo(() => {
  return assignedStaffRows.reduce((sum, row) => {
  ...
  try {
    ...
    const data = bestPurchasePlan.map((row: any) => ({
      ...
      "Line Total (£)": Number(row.lineTotal || 0),
...
const profitMarginValue =
  currentRevenueTotal > 0
    ? (((currentRevenueTotal || 0) -
        ((currentCostTotal || 0) + (totalLabourCost || labourCost || 0))) /
        (currentRevenueTotal || 1)) *
      100
    : 0;
...
const cheapestSupplierIndex = numericPrices.findIndex(
  ...
  <div className="inline-flex items-center gap-1 whitespace-nowrap">
    ...
    <input
      type="number"
      placeholder="0.00"
      value={supplierPrices[ingredientKey]?.[0] ?? ""}
      onChange={(e) => {
        ...
        setSupplierPrices((prev) => {
          ...
          const next = [...current];
          next[0] = val === "" ? undefined : parseFloat(val);
          return { ...prev, [ingredientKey]: next };
        });
      }}
      className={`w-20 rounded border px-2 py-1 text-right text-sm ${
        numericPrices[0] > 0 && numericPrices[0] === cheapestValue
          ? "border-green-500 bg-green-100"
          : "border-neutral-300"
      }`}
    />
    {cheapestSupplierIndex === 0 && (
