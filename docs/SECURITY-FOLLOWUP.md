...
- **`firestore.rules`** itself — now covers 21 of 28 collections (the original 6 plus 15 more with a clear, consistent app-code pattern), with an explicit `REQUIRES MANUAL POLICY DECISION` section naming the remaining 7 and why; two more collections (`restaurant_import_candidates`, `restaurant_translation_requests`) were added in later sessions, bringing coverage to 23.
- **`docs/FIRESTORE-SECURITY-AUDIT.md`** — a later verification pass: a security matrix across all covered collections, a fix to a real test-harness dependency conflict, two new test cases, and the exact, reproduced evidence that the emulator suite still cannot run in this environment (Java missing) — read this for the most current "has this actually been tested" answer.
...
- Rules tests exist, their dependencies now install cleanly (a real version-conflict defect was found and fixed — see `docs/FIRESTORE-SECURITY-AUDIT.md`), but they have never actually run — `java` is still not present in this environment, confirmed by the emulator's own startup failure, not just a `java -version` check. See `tests/firestore-rules/README.md` for how to run them for real once Java is available.
